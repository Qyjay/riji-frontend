...
import { extractTakenAt, formatDate, formatTime, detectPeriod } from '@/utils/exif'
    ...
    success: async (res: any) => {
      ...
      try {
        for (let i = 0; i < tempFilePaths.length; i++) {
          ...
          // H5 下可拿到原生 File 对象解析 EXIF；App 端没有 File 构造器，走文件时间兜底。
          // #ifdef H5
          if (rawFile instanceof File) {
            meta = (await extractTakenAt(rawFile)) as any
          } else
          // #endif
          if (typeof tempFiles[i]?.lastModifiedTime === 'number') {
            meta = { takenAt: tempFiles[i].lastModifiedTime, takenAtSource: 'lastModified', lat: undefined, lon: undefined }
          }
