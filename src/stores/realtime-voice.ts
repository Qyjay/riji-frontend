...
export const useRealtimeVoiceStore = defineStore('realtime-voice', () => {
  ...
  function handleServerEvent(event: VoiceServerEvent) {
    ...
    // #region debug-point stream-playback-store-event
    if ([
      ...
      fetch('http://127.0.0.1:7777/event', { method: 'POST', body: JSON.stringify({ sessionId: 'stream-playback-overlap', runId: 'post-fix', hypothesisId: 'B,E', location: 'stores/realtime-voice.ts:handleServerEvent', msg: '[DEBUG] realtime voice server audio event', data: { eventType: event.type, eventId: 'eventId' in event ? event.eventId : undefined, itemId: 'itemId' in event ? event.itemId : undefined, audioBytes: event.type === 'assistant.audio.delta' ? Math.floor(event.audio.length * 3 / 4) : 0, phase: phase.value, audioQueueSize: audioQueueSize.value, socketState: socketState.value, activeAssistantTextLength: activeAssistantText.value.length, activeUserTextLength: activeUserText.value.length }, ts: Date.now() }) }).catch(() => {})
    }
    // #endregion
    switch (event.type) {
      case 'session.ready':
